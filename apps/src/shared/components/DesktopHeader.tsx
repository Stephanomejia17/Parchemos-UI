...
export function DesktopHeader({
  ...
  return (
    <header className="hidden h-16 flex-shrink-0 items-center gap-4 border-b border-border bg-white px-6 md:flex">
      {backHref ? (
          ...
          return (
            <span key={`${crumb.label}-${index}`} className="flex min-w-0 items-center gap-1">
              <span className="truncate">
                {!current && crumb.href ? (
                  <Link href={crumb.href} className="text-gray-500 hover:text-primary">
                    {crumb.label}
                  </Link>
                ) : (
                  <span className={current ? "font-bold text-gray-900" : "text-gray-500"}>
                    {crumb.label}
                  </span>
                )}
              </span>
