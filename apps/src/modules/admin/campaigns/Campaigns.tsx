...
import { campaigns } from "@/mocks/admin/campaigns";
  ...
  return (
    <div className="space-y-5">
      <SectionHeader
        ...
        action={
          <button
            className="flex items-center gap-1.5 px-3 py-2 text-[13px] text-white rounded-xl"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {campaigns.map((c) => (
          <Card
            key={c.id}
            className="p-5 hover:shadow-[0_4px_16px_rgba(0,0,0,0.07)] transition-all"
          >
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                <div className="text-[14px] font-semibold text-gray-900">
                  {c.reach > 0 ? c.reach.toLocaleString("es-CO") : "—"}
                </div>
                <div className="text-[11px] text-gray-500 mt-0.5">Alcance</div>
              </div>
              <div className="bg-gray-50 rounded-xl p-3 text-center">
                <div className="text-[14px] font-semibold text-gray-900">
                  {c.conversions > 0 ? c.conversions.toLocaleString("es-CO") : "—"}
                </div>
            ...
            <div className="flex items-center gap-1.5 pt-3 border-t border-gray-50">
              <button className="flex-1 py-2 text-[12px] font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">
                Ver métricas
              </button>
              <button className="flex-1 py-2 text-[12px] font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">
                Editar
