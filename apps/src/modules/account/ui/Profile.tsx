...
export function Profile() {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(null);
    try {
      await profileService.fetch("/auth/me", { method: "PATCH", body: { fullName: fullName.trim(), phone: phone.trim(), city: city.trim(), profilePhotoUrl: photo } });
      await refreshUser();
  ...
  const requestDeletion = async () => {
    ...
    setError(null);
    try {
      const result = await profileService.fetch<{ message: string; deletionEffectiveAt: string }>("/auth/me", { method: "DELETE" });
      await refreshUser();
