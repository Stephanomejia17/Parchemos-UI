...
export function MenuManagement({ restaurantId: initialRestaurantId }: { restaurantId?: string }) {
  ...
  async function loadRestaurants() {
    setLoading(true);
    try {
      const data = await menuService.listMyRestaurants();
      setRestaurants(data);
      const preferred = initialRestaurantId
        ? data.find((item) => item.id === initialRestaurantId)
        : data[0];
  ...
  async function loadProducts(id: string) {
    setLoading(true);
    try {
      const response = await menuService.listProducts(id, {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const response = editing
        ? await menuService.updateProduct(editing.id, body)
        : await menuService.createProduct(restaurantId, body);
      if (image) {
        await menuService.uploadProductImage(response.data.id, image);
      }
  ...
  async function toggleFeatured(product: Product) {
    try {
      setError(null);
      await menuService.setFeatured(product.id, !product.featured);
      await loadProducts(restaurantId);
    } catch (err) {
      setError(messageFrom(err));
    }
  ...
  async function changeProductStatus(product: Product, action: Status) {
    ...
    try {
      setError(null);
      if (action === "inactivo") {
        await menuService.deactivateProduct(product.id);
      } else {
        await menuService.activateProduct(product.id);
      }
      await loadProducts(restaurantId);
      setNotice(
        action === "inactivo"
  ...
  return (
    <section className="bg-background flex flex-col gap-5">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Crea, edita y administra los productos de tu restaurante.
          </p>
        </div>
        <PrimaryButton onClick={openCreate} disabled={!hasRestaurants}>
      ...
      {notice && (
        <TemporaryMessage type="success" message={notice} onClose={() => setNotice(null)} />
      )}

      {!loading && !hasRestaurants ? (
        <div className="bg-white border border-border rounded-2xl p-8 text-center">
          <p className="font-semibold text-gray-900">Aún no tienes restaurantes registrados.</p>
          <p className="text-sm text-muted-foreground mt-1">
            Crea un restaurante para comenzar a construir tu menú.
          </p>
        </div>
      ) : (
        <>
          ...
          {selectedRestaurant && (
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">
                {selectedRestaurant.businessName} · {productsLabel}
              </p>
              <div
              ...
              >
                <button
                  ...
                  className={`rounded-lg p-2 ${view === "list" ? "bg-orange-50 text-primary" : "text-gray-500 hover:bg-gray-50"}`}
                >
                ...
                <button
                  ...
                  className={`rounded-lg p-2 ${view === "cards" ? "bg-orange-50 text-primary" : "text-gray-500 hover:bg-gray-50"}`}
                >
          ...
          {loading ? (
            ...
            <div className="bg-white border border-border rounded-2xl p-8 text-center">
              <p className="font-semibold text-gray-900">No hay productos con estos filtros.</p>
              <button
                onClick={openCreate}
                    ...
                    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                      {category.products.map((product) => (
                        <ProductCard
                          ...
                          onRemove={() => setConfirming({ product, action: "inactivo" })}
                          onReactivate={(product) => setConfirming({ product, action: "activo" })}
                        />
                      ))}
                    </div>
                  ) : (
      ...
      {formOpen && (
        <div
          className="fixed inset-0 z-30 flex items-center justify-center bg-black/35 p-4"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between gap-4 mb-5">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-primary">Menú</p>
            ...
            <form onSubmit={save} className="flex flex-col gap-4" noValidate>
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm resize-none"
                />
              </label>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setFormOpen(false)}
                  className="rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-semibold text-gray-700"
                >
                  Cancelar
                </button>
                <PrimaryButton
      ...
      {confirming && (
        <ConfirmModal
          ...
          confirm={() => void changeProductStatus(confirming.product, confirming.action)}
        />
      )}
    </section>
  );
...
function ProductCard({
  ...
  return (
    <article
      className={`relative rounded-2xl border bg-white p-4 shadow-sm ${product.status === "inactivo" ? "bg-gray-50 border-gray-200" : "border-border"}`}
    >
      <div className="absolute right-3 top-3">
        <button
          ...
          className={`flex h-8 w-8 items-center justify-center rounded-xl ${product.featured ? "bg-amber-50 text-amber-600" : "bg-gray-100 text-gray-500"}`}
        >
          <Star className={`h-4 w-4 ${product.featured ? "fill-amber-400" : ""}`} />
        </button>
      </div>
      <div className="flex gap-4 pr-10">
        <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-orange-50">
          {product.imageUrl ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          <span
            className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${product.status === "activo" ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}
          >
      ...
      <div className="mt-4 flex items-center justify-end gap-2 border-t border-border pt-3">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl bg-gray-100 px-3 py-2 text-xs font-semibold text-gray-700"
        >
...
function ProductRow({
  ...
  return (
    <article className="flex items-center gap-3 border-b border-border p-3 last:border-b-0">
      ...
      <button
        ...
        onClick={() => onFeatured(product)}
        aria-label={product.featured ? "Quitar destacado" : "Marcar destacado"}
        className={`rounded-xl p-2 ${product.featured ? "bg-amber-50 text-amber-600" : "bg-gray-100 text-gray-500"}`}
      >
        <Star className={`h-4 w-4 ${product.featured ? "fill-amber-400" : ""}`} />
      </button>
      <button
    ...
    >
      <div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-xl">
        ...
        <p className="mt-2 text-sm text-gray-600">
          {activating ? "¿Quieres volver a activar" : "¿Quieres desactivar"}{" "}
          <strong>{product.name}</strong>?{" "}
          {activating
            ? "Volverá a estar disponible en el menú."
            : "No se eliminará definitivamente y podrás activarlo después."}
        </p>
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            onClick={confirm}
            disabled={busy}
            className={`rounded-xl px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60 ${activating ? "bg-green-600" : "bg-red-600"}`}
          >
            {busy
              ? activating
                ? "Activando..."
...
function messageFrom(error: unknown): string {
  return error instanceof ApiError ? error.message : "No pudimos completar la operación.";
}
