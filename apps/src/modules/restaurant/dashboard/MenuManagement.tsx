...
export function MenuManagement({ restaurantId: initialRestaurantId }: { restaurantId?: string }) {
  ...
  async function loadRestaurants() {
    setLoading(true);
    try {
      const data = await menuService.fetch<Restaurant[]>("/restaurantes/mios");
      setRestaurants(data);
  ...
  async function loadProducts(id: string) {
    ...
    try {
      ...
      const response = await menuService.fetch<ProductPageResponse>(`/restaurantes/${id}/productos?${params}`);
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const response = editing
        ? await menuService.fetch<ProductResponse>(`/productos/${editing.id}`, { method: "PATCH", body })
        : await menuService.fetch<ProductResponse>(`/restaurantes/${restaurantId}/productos`, { method: "POST", body });
      if (image) {
        const upload = new FormData();
        upload.append("file", image);
        await menuService.upload<ProductResponse>(`/productos/${response.data.id}/imagen`, upload);
      }
  ...
  async function updateProduct(id: string, body: unknown, successMessage: string) {
    try {
      setError(null);
      await menuService.fetch<ProductResponse>(`/productos/${id}`, { method: "PATCH", body });
      await loadProducts(restaurantId);
  ...
  async function toggleFeatured(product: Product) {
    try {
      setError(null);
      await menuService.fetch<ProductResponse>(`/productos/${product.id}/destacado`, { method: "PATCH", body: { featured: !product.featured } });
      await loadProducts(restaurantId);
    } catch (err) {
      setError(messageFrom(err));
    }
  ...
  async function changeProductStatus(product: Product, action: Status) {
    ...
    try {
      setError(null);
      await menuService.fetch<ProductResponse>(`/productos/${product.id}`, action === "inactivo" ? { method: "DELETE" } : { method: "PATCH", body: { status: "activo" } });
      await loadProducts(restaurantId);
