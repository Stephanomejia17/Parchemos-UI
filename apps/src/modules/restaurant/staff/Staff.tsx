...
import type {
  Location,
  Restaurant,
  StaffAccountStatus as AccountStatus,
  StaffMember,
...
function StaffManager() {
  ...
  const [error, setError] = useState<string | null>(null),
    ...
    [selected, setSelected] = useState<StaffMember | null>(null);
  const load = useCallback(async () => {
    try {
      const [people, restaurants] = await Promise.all([
        restaurantService.listStaff(),
        restaurantService.listMyRestaurants(),
      ]);
      setStaff(people);
      setRestaurants(restaurants);
      setLocations(
        restaurants.flatMap((item) => item.locations).filter((item) => item.status === "activa"),
      );
    } catch (cause) {
      setError(errorMessage(cause, "No pudimos cargar el personal."));
    }
  }, []);
  useEffect(() => {
    void load();
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const created = await run(
      () =>
        restaurantService.createStaffMember({
  ...
  const setEnabled = (member: StaffMember, enabled: boolean) =>
    run(
        ...
        <header className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Crea cuentas, asigna sedes y controla el acceso de tu equipo.
            </p>
          </div>
          <PrimaryButton onClick={() => setCreateOpen(true)} disabled={!locations.length}>
        ...
        <SurfaceCard className="overflow-hidden bg-white">
                  ...
                  <tr key={member.id} className="border-t">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <IconAction label="Ver detalle" onClick={() => setSelected(member)}>
          ...
          {!staff.length && (
            <div className="p-10 text-center text-sm text-muted-foreground">
              <UsersRound className="mx-auto mb-2 h-7 w-7 text-primary" />
              Aún no has creado cuentas de personal.
            </div>
          )}
        </SurfaceCard>
        {createOpen && (
    ...
    <Modal title="Crear cuenta de personal" close={close}>
      <form onSubmit={submit} className="space-y-3">
        ...
        <p className="rounded-lg bg-amber-50 p-2 text-xs text-amber-800">
          Comparte la contraseña inicial con el integrante por un canal seguro.
        </p>
        <PrimaryButton type="submit" disabled={busy || !locations.length}>
...
function DetailModal({
  ...
  const reassign = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const locationId = String(new FormData(event.currentTarget).get("locationId"));
    const ok = await run(
      () => restaurantService.reassignStaffLocation(member.id, locationId),
      "No pudimos reasignar la sede.",
    );
    if (ok) close();
  };
  return (
    <Modal title={editing ? "Editar cuenta" : "Detalle del personal"} close={close}>
      {editing ? (
...
function Input({
  ...
  return (
    <label className="block text-sm font-medium">
      {label}
      <input {...props} className="mt-1 w-full rounded-xl border p-2.5 font-normal" />
      {hint && <small className="mt-1 block font-normal text-xs text-gray-500">{hint}</small>}
    </label>
  );
}
...
function Status({ status }: { status: AccountStatus }) {
  return (
    <span
      className={`rounded-full px-2 py-1 text-xs font-medium ${status === "activa" ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-600"}`}
    >
      {status === "activa" ? "Activa" : "Deshabilitada"}
    </span>
  );
}
function IconAction({
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-auto bg-black/30 p-4">
      <SurfaceCard className="mx-auto my-6 max-w-lg bg-white p-5">
...
function Alert({ text, close }: { text: string; close: () => void }) {
  return (
    <div
      role="alert"
      className="fixed left-1/2 top-5 z-[60] flex w-[min(92vw,42rem)] -translate-x-1/2 justify-between rounded-xl bg-red-50 p-3 text-sm text-red-700 shadow-lg"
    >
      <span>
