...
function StaffManager() {
  const [staff, setStaff] = useState<StaffMember[]>([]), [restaurants, setRestaurants] = useState<Restaurant[]>([]), [locations, setLocations] = useState<Location[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [people, restaurants] = await Promise.all([
        restaurantService.fetch<StaffMember[]>("/restaurantes/personal"), restaurantService.fetch<Restaurant[]>("/restaurantes/mios"),
      ]);
      setStaff(people);
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const created = await run(() => restaurantService.fetch("/restaurantes/personal", { method: "POST", body: {
...
function DetailModal({ member, locations, busy, close, run }: { member: StaffMember; locations: Location[]; busy: boolean; close: () => void; run: (action: () => Promise<unknown>, fallback: string) => Promise<boolean> }) {
  ...
  const update = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const ok = await run(() => restaurantService.fetch(`/restaurantes/personal/${member.id}`, { method: "PATCH", body: { fullName: String(form.get("fullName")).trim(), phone: String(form.get("phone")).trim() } }), "No pudimos editar la cuenta."); if (ok) { setEditing(false); close(); } };
  const reassign = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const locationId = String(new FormData(event.currentTarget).get("locationId")); const ok = await run(() => restaurantService.fetch(`/restaurantes/personal/${member.id}/sede`, { method: "PUT", body: { locationId } }), "No pudimos reasignar la sede."); if (ok) close(); };
...
function SelectLocation({ locations, defaultValue, disabled = false }: { locations: Location[]; defaultValue?: string; disabled?: boolean }) { return <select required disabled={disabled} name="locationId" defaultValue={defaultValue ?? ""} className="mt-1 w-full rounded-xl border p-2.5"><option value="" disabled>Selecciona una sede</option>{locations.map(location => <option key={location.id} value={location.id}>{location.name}</option>)}</select>; }
